...
class CopilotInlineProvider implements IInlineCompletionProvider {
  ...
  readonly identifier = 'khulnasoft_jupyter:provider';
  readonly rank = 1000;
...
/**
 * Initialization data for the khulnasoft_jupyter extension.
 */
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'khulnasoft_jupyter:plugin',
  ...
  activate: (
    ...
    const command = 'khulnasoft_jupyter:completion';
    ...
    notebookTracker.widgetAdded.connect(async (_, notebook) => {
      ...
      const wsURL = URLExt.join(serverSettings.wsUrl, 'khulnasoft-jupyter', 'ws');
